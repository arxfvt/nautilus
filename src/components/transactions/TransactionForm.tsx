...
export function TransactionForm({ onSuccess }: TransactionFormProps) {
  ...
  const currency = selectedAccount?.currency ?? "UGX";
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col flex-1 min-h-0 overflow-hidden">
      {/* Type tabs */}
      <div
        className="flex rounded-xl p-1 mx-4 mt-2 mb-4"
      ...
      {/* Error + Save — always visible, pinned to bottom */}
      <div className="px-4 pb-8 pt-2 flex-shrink-0 space-y-2">
        {error && (
          <p className="text-xs text-center py-1 px-3 rounded-lg" style={{ background: "var(--color-expense)18", color: "var(--color-expense)" }}>
            {error}
          </p>
        )}
        <button
        ...
        >
          {loadingData ? "Loading…" : loading ? "Saving…" : "Save"}
        </button>
      </div>
    </form>
  );
